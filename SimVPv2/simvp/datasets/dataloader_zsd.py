...
class ZsdDataset(Dataset):
    def __init__(self, data_dir, training_time, seq_len=10, out_seq_len=1, surface_mask=True, mean=None, std=None):
        ...
        if surface_mask:
            ...
            self.data = self.data*self.surface_mask[:self.data.shape[-2], :self.data.shape[-1]]
        ...
        self.valid_idx = np.array(
            range(0, (self.data.shape[0]- out_seq_len - self.seq_len) * self.num_patches_per_image))
    def __len__(self):
    ...
    def __getitem__(self, index):
        ...
        while True:
            ...
            y_data = torch.tensor(self.data[idx+self.seq_len:idx+self.seq_len+self.out_seq_len, :, lon_i:lon_i+self.patch_h, lat_i:lat_i+self.patch_w]).to(torch.float32)
